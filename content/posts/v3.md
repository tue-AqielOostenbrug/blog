...
draft = false 
title = 'Draft: Honoring the IRC Protocol v3'
toc = true
+++
...
- [] Produce a complete embedded system, so I can apply my new knowledge to industry (design, build, test and document the system)
- [] Stay organized (**not covered in this post**)
- [ ] ~~Fly to Mount Doom~~
...
Now that I felt like I was back in the game, I could finally properly tackle the IRC socket communication in C. Eventually, after a good amount of reading and trial and error, I ended up with a working snippet.

While working on the snippet, I after found myself in a situation were the server would ignore my commands. After debugging, I concluded that this seemed to happen due to one of two reasons. My stupid mistakes include but are not limited to:
...
Hence, I eventually decided to tried to look at the problem from a different angle... I decided to mix the general ESP32 examples and jingcai examples to get a C project with lvgl working, that showed the GUI from jingcai while using provided wifi features form esp-idf.

However, due to the aforementioned issues. This wasn't easy... I started with the sdkconfig. I concatenated the sdkconfigs from both projects and then removed all of the duplicates. Next, I started moving all the inclusion paths from the component.mk and Makefiles  files to local CMakeList.txt files with help of ChatGPT. Then, I added the missing components of the Jingcai example to the esp-idf example. And also made them CMakeList files as explained before. Finally, I adjusted the toplevel CMakeList for the previous changes. Added an idf\_component.yml.

After all of that it started compiling with less errors. However, many GUI driver errors remained. So, I just started removing many unused ones until the errors stopped.
...
### Depression
Although I really didn't want to into the touchscreen driver. I didn't really have a choice anymore. So I opened the driver and first started analyzing every global variable and then every function, while making sure to translate all Chinese commments I encountered on the way.  After analyzing the driver on a high level, I located the function that returns the touch coordinates (**NOT HOW I REMEMBER THIS, I think the files have changed as well!!!**) **THE ISSUE MIGHT HAVE OCCURED BECAUSE OF THE MANUAL INTEGRATION I DID  BECAUSE THE FUNCTION WAS UNUSED**:
...
```
So I tried all combinations of the commented out lines. When this didn't result in a change of orientation, I removed the who lines for flipping x and y and then replaced the flip coordinates function with the unused xpt2046_corr(x,y) function??? Even after all of that I could not get it to work.
...
- [x]*Become an intermediate C programmer*: By going back to the basics i.e. reading up on C in books and courses I have strengthened by foundamentals. Additionally, by programming embedded systems using the esp-idf and C, I received practical experience. This allowed me to become an intermediate C programmer.
- [x] Produce a complete embedded system: During this journey, I went to through the entire development cycle. Thus, I can now handle a larger part of the production pipeline of embedded systems.
- [x] Stay organized: For this journey, I used Scrum and worked in sprints to increase the speed of iterations and to include feedback quicker. This helped me stay organized.
...
Still, I think that this project was only the start. I still have many things I  want to improve and add to this project, such as messaging over BlueTooth and LoRa, or turning the ESP32 into a portable IRC server, and many other things.

Nevertheless, for now, I want to thank my Professional supervisor for all the advice and positivity even when I kept swearing about package incompatibility and [not being a real programmer](https://homepages.inf.ed.ac.uk/rni/papers/realprg.html). I also want to thank my Personal Leadership supervisor Esmee van der Ham for mentally preparing me for the journey. Finally, I want to thank the reader for bearing with me until the end.
